...
@Component({
  selector: "entry-voucher",
  templateUrl: "./entry-voucher.component.html",
  styleUrls: ["./entry-voucher.component.css"],
})
export class EntryVoucherComponent implements OnInit {
  ...
  constructor(
    ...
    private transfertLineServie: LineRequestTransfertService,
  ...
  async getTotal() {
    await this.requestTransfertService
      .getTotalRequestById(this.idRequest)
      .then((data) => {
        this.total = data;
      });
  }
  ...
  async getDrivers() {
    await this.userService
      .getDriversByIdRequest(this.idRequest)
      .then((data) => {
        this.drivers = data;
      });
  }
  ...
  async getEntryVoucher() {
    await this.entryVoucherService
      .getEntryVoucherById(this.requestTransfert.entryVoucher)
      .then((data) => {
  ...
  async getAllLineDetails() {
    await this.transfertLines.forEach(async (data) => {
      await this.lineDetailTransfertService
        .findAllTransfertLineDetaiByIdLineTransfert(data.id)
        .then((ld) => {
  ...
  async getLT() {
    await this.transfertLineServie
      .getAllLineRequestTransfertByIdRequestWithPromise(this.idRequest)
      .then((data) => {
        data.forEach((lt) => {
          if (lt.state == 3 || lt.state==4 || lt.state==5) {
            this.transfertLines.push(lt);
          }
        });
      });
  }
  ...
  async getUser() {
    await this.userService
      .getUserByIdWithPromise(this.requestTransfert.inventaryManagerId)
      .then((data) => {
        this.user = data;
      });
  }
  ...
  public downloadAsPDF() {
    const htmlToPrint = "" + '<style type="text/css">' + "</style>";
    var printContents = document.getElementById("pdf").innerHTML;
    var popupWin = window.open("Angular Large Table to pdf", "_blank");
